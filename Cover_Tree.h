 ...
 * queries.
 *
 * The user should define float Point::distance(const Point& p) and
...
class CoverTree
{
    ...
    public:
        ...
        float distance(const CoverTreeNode& p) const;
 ...
 private:
    typedef std::pair<float, CoverTreeNode*> distNodePair;
...
template<class Point>
CoverTree<Point>::CoverTree(const float& maxDist,
                            const std::vector<Point>& points)
{
    _root=NULL;
    _numNodes=0;
    _maxLevel=ceil(log(maxDist)/log(base));
...
CoverTree<Point>::kNearestNodes(const Point& p, const unsigned int& k) const
{
    ...
    float maxDist = p.distance(_root->getPoint());
    ...
    for(int level = _maxLevel; level>=_minLevel;level--) {
        ...
        for(int i=0; i<size; i++) {
            ...
            for(it2=children.begin(); it2!=children.end(); ++it2) {
                float d = p.distance((*it2)->getPoint());
        ...
        float sep = maxDist + pow(base, level);
        size = Qj.size();
        for(int i=0; i<size; i++) {
            if(Qj[i].first > sep) {
...
bool CoverTree<Point>::insert_rec(const Point& p,
                                  ...
                                  const int& level)
{
    std::vector<std::pair<float, CoverTreeNode*> > Qj;
    float sep = pow(base,level);
    float minDist = FLT_MAX;
    std::pair<float,CoverTreeNode*> minQiDist(FLT_MAX,NULL);
    typename  std::vector<std::pair<float, CoverTreeNode*> >::const_iterator it;
        ...
        for(it=children.begin();it!=children.end();++it) {
            ...
            while(true) {
                ...
                for(it2=Q.begin();it2!=Q.end();++it2) {
                    float d = q.distance(it2->second->getPoint());
                    if(d<minDQ) {
                        minDQ = d;
...
template<class Point>
std::pair<float, typename CoverTree<Point>::CoverTreeNode*>
                           ...
                           const std::vector<CoverTreeNode*>& Q)
{
    ...
    for(it=Q.begin();it!=Q.end();++it) {
        float dist = p.distance((*it)->getPoint());
        if(dist < minDist) {
            minDist = dist;
            minNode = *it;
        }
...
void CoverTree<Point>::insert(const Point& newPoint)
{
    ...
    if(newPoint.distance(n->getPoint())==0.0) {
        ...
        insert_rec(newPoint,
                   ...
                   _maxLevel);
    } //all of this is uneccessary since our points should always be distinct
}
...
void CoverTree<Point>::remove(const Point& p)
{
    ...
    //std::cout<<"Dealt with root removal\n";
    std::map<int, std::vector<distNodePair> > coverSets;
    //std::cout<<"1\n";
...
void CoverTree<Point>::CoverTreeNode::removePoint(const Point& p)
{
    typename std::vector<Point>::iterator it =
        std::find(_points.begin(), _points.end(), p);
    if(it != _points.end())
        _points.erase(it);
}

template<class Point>
float CoverTree<Point>::CoverTreeNode::distance(const CoverTreeNode& p) const
{
    ...
    for(int i=_maxLevel;i>_minLevel;i--) {
        ...
        for(it=nodes.begin(); it!=nodes.end(); ++it) {
            for(it2=nodes.begin(); it2!=nodes.end(); ++it2) {
                float dist=(*it)->distance((*it2)->getPoint());
        ...
        for(it=nodes.begin(); it!=nodes.end(); ++it) {        
            ...
            for(it2=children.begin(); it2!=children.end(); ++it2) {
                float dist = (*it2)->distance((*it)->getPoint());
                if(dist>sep) {
